...
function setup(ctx) {
  ...
  function renderObsidianSettings(state) {
    ...
    form.appendChild(createField("Lore tag", createTextInput(characterDraft.obsidianLoreTag, "e.g. lore (required — only tagged notes sync)", (next) => {
      characterDraft.obsidianLoreTag = next;
    })));
    form.appendChild(createFieldNote(characterDraft.obsidianLoreTag ? `Only notes tagged #${characterDraft.obsidianLoreTag} (or a nested tag) sync. Notes without it are skipped, and any previously synced are removed on the next sync.` : "Set a lore tag (e.g. lore) and tag your notes with it — only tagged notes sync. With no tag, nothing syncs."));
